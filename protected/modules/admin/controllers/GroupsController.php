...
class GroupsController extends Controller
{
   ...
   /**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Groups']))
		{
         ...
         if($model->validate()){
            ...
            if(isset($_POST['Groups']['functional'])){
               foreach($_POST['Groups']['functional'] as $value){
                  $group_functional = new GroupFunctional();
                  $group_functional->functional_id = $value;
                  $group_functional->group_id = $model->id;
                  $group_functional->save();      
               }
            }
         }
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Groups']))
		{
            ...
            //Cập nhật vào bảng member_group
            if(isset($_POST['Groups']['functional'])){
               GroupFunctional::model()->deleteAllByAttributes(array(
                  'group_id' => $id
               ));
               foreach($_POST['Groups']['functional'] as $value){
                  $group_functional = new GroupFunctional();
   ...
   public function gridFunctional($data,$row){
      //List functional
      $db = Yii::app()->db;
      $sql = "SELECT name FROM functional A INNER JOIN group_functional B ON A.id = B.functional_id WHERE group_id = " . $data->id;
