...
class Mobile_Detect
{
    ...
    public function getIOSVersion(){
        $user_agent = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : null;
        if($user_agent != null){
            preg_match("/iPhone OS [0-9_]*/", $user_agent, $output_array);
            //CVarDumper::dump($output_array,10,true);die;
            if(isset($output_array[0])){
                $version = str_replace(array('iPhone OS ','_'),array('','.'),$output_array[0]);
    ...
    public function getAndoidVersion(){
        $user_agent = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : null;
        if($user_agent != null){
            preg_match("/Android [0-9.]*/", $user_agent, $output_array);
            //CVarDumper::dump($output_array,10,true);die;
            if(isset($output_array[0])){
                $version = str_replace('Android ','',$output_array[0]);
                return floatval($version);
            }else{
                return false;
            }
