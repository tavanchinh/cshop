<?php 
class MyDatetime{
   public static function getchinesedate($year,$month,$day){
   ...
   return $cdate_result;
	...
	public static function getTimeAgo($time_stamp){
		$today = getdate();
		$mday = $today['mday'];
		$year = $today['year'];
		//echo intval(date('d',$time_stamp)) + 1 ;die();
		if($mday > (intval(date('d',$time_stamp)) + 1)){ // Nếu khoảng time > 2 ngày về trước
		$year_cmt = (date('Y',$time_stamp) == $year) ? '' : ' năm ' . date('Y',$time_stamp);

		return date('d',$time_stamp) . ' tháng ' . date('m',$time_stamp) . $year_cmt . ' lúc ' . date('H:i',$time_stamp);
		}
		if($mday > intval(date('d',$time_stamp))){ // Nếu time là ngày hôm qua
		return 'Hôm qua lúc ' . date('H:i',$time_stamp);
		}
		if($mday == date('d',$time_stamp)){ // Nếu trong một ngày
		$sec_ago = time() - $time_stamp;
		if($sec_ago < 60){ // Nếu trong một phút
		   return 'Vừa xong';
		}
		else if($sec_ago < 3600){ // Nếu trong một giờ
		   return floor($sec_ago/60) . ' phút trước';
		}
		return floor($sec_ago/3600) . ' giờ trước';

		}
		return date('d/m/Y H:i',$time_stamp); // Cac truong hop con lai
	}
}
?>